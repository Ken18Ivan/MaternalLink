...
const PatientsPage = () => {
 ...
 const fetchPatients = async () => {
  ...
  // --- HANDLE CLINIC SUBMIT (WITH VALIDATION) ---
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-blue-600 p-6 flex justify-between items-center text-white shrink-0">
              ...
              <div className="flex gap-2">
                <button onClick={generatePDF} className="bg-white/20 p-2 rounded-full"><Download className="w-5 h-5" /></button>
                <button onClick={() => setSelectedPatient(null)} className="bg-white/20 p-2 rounded-full"><X className="w-5 h-5" /></button>
              </div>
            </div>
              ...
              <div className="bg-blue-50 border-2 border-blue-100 p-5 rounded-2xl mb-8">
                ...
                <div className="mt-4 pt-4 border-t border-blue-100">
                  ...
                  <form onSubmit={handleSendFeedback} className="space-y-2">
                    <textarea required placeholder="Message..." className="textarea w-full h-20 rounded-xl bg-white border-gray-300 text-black" value={feedback} onChange={(e) => setFeedback(e.target.value)}></textarea>
                    <button className="btn btn-sm w-full bg-blue-600 text-white"><Send className="w-4 h-4" /> Send SMS</button>
                  </form>
                </div>
              </div>
